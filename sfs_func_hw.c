...
void cd_inode(struct sfs_inode inode, const char* path) {
	int i;
	...
	if (inode.sfi_type == SFS_TYPE_DIR) {
		for(i=0; i < SFS_NDIRECT; i++) {
			...
			if(cd_directory(dir_entry, path)) break;
		}
	}
}

int cd_directory(struct sfs_dir dir_entry[], const char* path) {
	int i;
	struct sfs_inode inode;
	char cd_path[SFS_NAMELEN];

	for(i=0; i < SFS_DENTRYPERBLOCK;i++) {
		disk_read(&inode,dir_entry[i].sfd_ino);
		if(!strcmp(path, dir_entry[i].sfd_name)){	// find path is exist
			if(inode.sfi_type == SFS_TYPE_DIR) sd_cwd = dir_entry[i];
...
void sfs_cd(const char* path)
{
	...
	if(path != NULL){	// if path is NULL, do nothing
		disk_read(&c_inode, sd_cwd.sfd_ino);	
		cd_inode(c_inode, path);
	}
}

void ls_cd_inode(struct sfs_inode inode, const char* path) {
	int i;
	struct sfs_dir dir_entry[SFS_DENTRYPERBLOCK];
	...
	if (inode.sfi_type == SFS_TYPE_DIR) {
		for(i=0; i < SFS_NDIRECT; i++) {
			if (inode.sfi_direct[i] == 0) break;
			disk_read(dir_entry, inode.sfi_direct[i]);
			if(ls_cd_directory(dir_entry, path)) break;
		}
	}
}

int ls_cd_directory(struct sfs_dir dir_entry[], const char* path) {
	int i;
	struct sfs_inode inode;
	char cd_path[SFS_NAMELEN];
	...
	for(i=0; i < SFS_DENTRYPERBLOCK;i++) {
		disk_read(&inode,dir_entry[i].sfd_ino);
		if(!strcmp(path, dir_entry[i].sfd_name)){	// find path is exist
			if(inode.sfi_type == SFS_TYPE_DIR) ls_inode(inode);
			else printf("%s\n", path);
			return 1;
		}
		else continue;
...
void ls_inode(struct sfs_inode c_inode){
	int i, j;
	struct sfs_inode inode;
	struct sfs_dir dir_entry[SFS_DENTRYPERBLOCK];
	...
	if (c_inode.sfi_type == SFS_TYPE_DIR) {
		for(i=0; i < SFS_NDIRECT; i++) {
			...
			for(j=0; j < SFS_DENTRYPERBLOCK;j++) {
				...
				if (inode.sfi_type == SFS_TYPE_DIR) printf("%s/\t", dir_entry[j].sfd_name);
				else if (inode.sfi_type == SFS_TYPE_FILE) printf("%s\t", dir_entry[j].sfd_name);
				else break;
			}
		}
	}
...
void sfs_ls(const char* path)
{
	...
	if(path == NULL) ls_inode(c_inode); // if path is null
	else ls_cd_inode(c_inode, path); //else path is not null
}
